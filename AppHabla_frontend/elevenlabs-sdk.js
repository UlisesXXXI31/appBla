...
Also see the ElevenAgents overview
Installation
...
Upgrading from an earlier version? Run npx skills add elevenlabs/packages to install the elevenlabs:sdk-migration skill for your AI coding agent, which automates import changes and API updates.
...
Public agents
Agents that don’t require any authentication can be used to start a conversation by using the agent ID. The agent ID can be acquired through the ElevenLabs UI.
...
The connection type is automatically inferred based on the conversation mode. Voice conversations use WebRTC and text-only conversations use WebSocket by default. You can still explicitly specify connectionType: 'webrtc' or connectionType: 'websocket' if needed.
